        ...
        <div class="max-w-7xl mx-auto px-6 sm:px-6 lg:px-8">
            @if ($favourites->isEmpty())
                <div class="text-center text-lg text-gray-700 py-24">
                    ...
                    <a href="{{ route('products') }}" class="text-blue-600 hover:underline">Browse products</a> and add some items!
                </div>
            @else
                <!-- Favourite Items -->
                <div class="space-y-6">
                        ...
                        <div class="flex items-center justify-between p-4 border rounded-md shadow-sm">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">Price: ${{ number_format($favourite->product->price, 2) }}</p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-4">
                                <form action="{{ route('favourit.remove', $favourite->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
